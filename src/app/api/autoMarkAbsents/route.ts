...
export async function GET(request: Request) {
  ...
  if (isDebugMode) {
    console.log('[API autoMarkAbsents] DEBUG MODE ENABLED');
  }
  
  // Check if time override is provided (for testing)
  const timeOverride = request.headers.get('x-time-override');
  
  // 2. Implement your logic (similar to your current Firebase Function)
  try {
    // Use overridden time if provided (for testing purposes only)
    const now = timeOverride ? new Date(timeOverride) : new Date();
    
    // Check if a specific day override is provided (for testing)
    const dayOverride = request.headers.get('x-day-override');
    
    if (timeOverride && isDebugMode) {
      console.log(`[API autoMarkAbsents] Using time override: ${timeOverride}`);
      console.log(`[API autoMarkAbsents] Simulated time: ${now.toISOString()}`);
    }
    
    const todayStr = now.toISOString().slice(0, 10); // YYYY-MM-DD
    const daysOfWeek = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    // Use day override if provided, otherwise use the day from the date
    const todayDay = dayOverride || daysOfWeek[now.getDay()];
    
    if (dayOverride && isDebugMode) {
      console.log(`[API autoMarkAbsents] Using day override: ${dayOverride}`);
    }
    ...
    for (const classroomDoc of classroomsSnap.docs) {
      ...
      for (const session of sessions) {
        ...
        // Extra debug logging
        if (isDebugMode) {
          console.log(`[API autoMarkAbsents] DEBUG - Session details for ${classCode}:`);
          console.log(`[API autoMarkAbsents] DEBUG - Session day: ${sessionDay}, Today: ${todayDay}, Match: ${sessionDay === todayDay}`);
          console.log(`[API autoMarkAbsents] DEBUG - Session end: ${sessionEnd.toISOString()}, Now: ${now.toISOString()}`);
          console.log(`[API autoMarkAbsents] DEBUG - Time since end: ${timeDiffFromEnd / 60000} minutes`);
          console.log(`[API autoMarkAbsents] DEBUG - Window: ${thirtyMinutesInMillis / 60000} minutes`);
          console.log(`[API autoMarkAbsents] DEBUG - Is after class ended: ${isAfterClassEnded}`);
          console.log(`[API autoMarkAbsents] DEBUG - Condition checks:`, {
            'now > sessionEnd': now > sessionEnd,
            'timeDiffFromEnd < thirtyMinutesInMillis': timeDiffFromEnd < thirtyMinutesInMillis,
            'timeDiffFromEnd >= 0': timeDiffFromEnd >= 0
          });
          console.log(`[API autoMarkAbsents] DEBUG - Full session object:`, session);
        }
        ...
        if (isAfterClassEnded) {
          ...
          for (const studentDoc of studentsSnap.docs) {
            ...
            if (isDebugMode) {
              console.log(`[API autoMarkAbsents] DEBUG - Attendance check for studentId=${studentId}, classCode=${classCode}: found ${attendanceSnap.size} records`);
            }
    ...
    if (isDebugMode) {
      ...
      response['debug'] = {
        ...
        classroomsFound: classroomsSnap.size
      };
    }
